    ...
    def setup(self):
        for host in self.net.hosts:
            ...
            if host.name.startswith('producer'):
                print "run producer"
                host.cmd('ndnputchunks -v /ndn/edu/producer/gpl3 < /usr/share/common-licenses/GPL-3 > producer_log &')
    ...
    def run(self):
        for host in self.net.hosts:
            if host.name.startswith('consumer'):
                print "run consumer"
                host.cmd("ndncatchunks -d iterative /ndn/edu/producer/gpl3 > gpl3 &")
                time.sleep(2)
