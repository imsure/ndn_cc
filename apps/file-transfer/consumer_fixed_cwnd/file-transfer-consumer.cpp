...
main(int argc, const char *argv[])
{
  ...
  std::fstream ofs;
  int cwnd_size = 5;
  int interest_life_time = 2000; // ms
  ...
  desc.add_options()
    ...
    ("cwnd,c", po::value<int>(&cwnd_size)->default_value(cwnd_size), "window size")
  ...
  try {
    Consumer consumer(prefix, file_name, cwnd_size, interest_life_time, is_verbose);
    consumer.run();
