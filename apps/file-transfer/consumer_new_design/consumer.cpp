...
Consumer::sendInterest(uint64_t segno, bool retx)
{
  ...
  interest.setInterestLifetime(time::milliseconds((int)(m_rto * 1.2)));
  interest.setMustBeFresh(true);
...
Consumer::onTimeout(const Interest& interest)
{
  ...
  //std::cout << "Interval between two time out: " << interval.count() << "ms" << std::endl;
  ...
  if (m_isVerbose) {
    std::cout << "Interest " << segno << " was timed out" << std::endl;
